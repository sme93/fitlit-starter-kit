...
const displayAllInfo = (event) => {
  const currentUser = event.target.id 
...
const displayFriends = () => {
  ...
  const markup = user.friends.reduce((acc, friend) => {
    ...
    acc += `<p class="friend-names" id=${currentFriend.id}> 
      ${currentFriend.name}</p>`
    return acc;
  }, "");
...
const displayAllUserAvgs = () => {
  ...
  <i class="fas fa-circle fa-1x" id="userCircle"></i>
    ...
    <p class="card-title">MINUTES ACTIVE</p>
      <p class="comparisons">Yours: ${activity
    .calculateMinutesActive(user, "2019/09/22")} minutes
      <br>All Users: ${activity
    .calculateAllUserActivityAvg("2019/09/20")} minutes</p>
      <br>
